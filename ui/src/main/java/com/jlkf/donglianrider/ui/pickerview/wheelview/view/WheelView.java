package com.jlkf.donglianrider.ui.pickerview.wheelview.view;
...
import com.jlkf.donglianrider.ui.pickerview.wheelview.adapter.WheelAdapter;
import com.jlkf.donglianrider.ui.pickerview.wheelview.interfaces.IPickerViewData;
import com.jlkf.donglianrider.ui.pickerview.wheelview.listener.LoopViewGestureListener;
import com.jlkf.donglianrider.ui.pickerview.wheelview.listener.OnItemSelectedListener;
import com.jlkf.donglianrider.ui.pickerview.wheelview.timer.InertiaTimerTask;
import com.jlkf.donglianrider.ui.pickerview.wheelview.timer.MessageHandler;
import com.jlkf.donglianrider.ui.pickerview.wheelview.timer.SmoothScrollTimerTask;
...
public class WheelView extends View {
    ...
    private Paint paintLableText;
    private Paint paintCustomText;//最大强度
    private Paint paintCustomBg;//最大强度
    ...
    private boolean isShowMaxText = false;//是否显示最大强度
    private boolean customTextVisibility = true;//是否显示最大强度
    ...
    private void initPaints() {
        ...
        paintLableText = new Paint();
        paintLableText.setColor(Color.parseColor("#0f0f0f"));
        paintLableText.setTextSize((int) (18 * fontScale + 0.5f));
        paintCustomText = new Paint();
        paintCustomText.setColor(Color.parseColor("#f5a623"));
        paintCustomText.setTextSize((int) (13 * fontScale + 0.5f));

        paintCustomBg = new Paint();
        paintCustomBg.setColor(Color.parseColor("#20f5a623"));
        paintCustomBg.setColor(Color.parseColor("#20f5a623"));

        //修改选中行的颜色
        paintCenterTextBg = new Paint();
//        paintCenterTextBg.setColor(Color.parseColor("#313A49"));
        //todo
        paintCenterTextBg.setColor(Color.parseColor("#ffffff"));
    ...
    private void remeasure() {//重新测量
        ...
        Log.d("data数据第一条线=", firstLineY+"");
        Log.d("data数据第二条线=", secondLineY+"");
        Log.d("data数据item=", itemHeight+"");
    ...
    public final void setMaxTextShow(boolean isShow) {
        isShowMaxText = isShow;
        invalidate();
    }

    public final void setCustomTextVisibility(boolean customTextVisibility){
        this.customTextVisibility = customTextVisibility;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (dividerType == DividerType.WRAP) {//横线长度仅包裹内容
            ...
            canvas.drawRect(startX, firstLineY, endX, secondLineY, paintCenterTextBg);
        } else {
//            canvas.drawLine(0.0F, firstLineY, measuredWidth, firstLineY, paintIndicator);
            canvas.drawLine(0.0F, firstLineY, measuredWidth, firstLineY, paintIndicator);
            canvas.drawLine(0.0F, secondLineY, measuredWidth, secondLineY, paintIndicator);
            canvas.drawRect(0.0F, firstLineY, measuredWidth, secondLineY, paintCenterTextBg);
        }
...
//            int drawRightContentStart = measuredWidth * 3 / 4 - getTextWidth(paintCenterText, label);
            int drawRightContentStart = measuredWidth / 2 + getTextWidth(paintCenterText, label) - 10;
            canvas.drawText(label, drawRightContentStart - CENTER_CONTENT_OFFSET, centerY, paintLableText);
        }

        //---------------自定义  显示最大强度-----------------
        if (isCenterLabel && isShowMaxText) {
            //绘制文字，靠右并留出空隙
            int drawRightContentStart = measuredWidth - getTextWidth(paintCustomText, "最大强度") - 100;
            canvas.drawText("最大强度", drawRightContentStart - CENTER_CONTENT_OFFSET + 20, centerY, paintCustomText);
            canvas.drawRect(drawRightContentStart - CENTER_CONTENT_OFFSET+10, centerY + 15, measuredWidth - 80, centerY - 40, paintCustomBg);
        }
